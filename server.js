...
function getOpsStatus(op) {
    if (op.features.length === 0)
        return op.status;
    var hasFailed = op.features.some(function (feature) { return feature.status === "failed"; });
    var allDone = op.features.every(function (feature) { return feature.status === "done"; });
    if (allDone)
        return op.status === "failed" ? "failed" : "completed";
    if (hasFailed && op.status === "completed")
        return "running";
    return op.status;
}
...
function upsertFeature(op, feature) {
    var _a, _b, _c;
    var idx = op.features.findIndex(function (item) { return item.id === feature.id; });
    if (idx >= 0) {
        op.features[idx] = __assign(__assign(__assign({}, op.features[idx]), feature), { toolActivity: (_b = (_a = feature.toolActivity) !== null && _a !== void 0 ? _a : op.features[idx].toolActivity) !== null && _b !== void 0 ? _b : [] });
        return;
    }
    op.features.push(__assign(__assign({}, feature), { toolActivity: (_c = feature.toolActivity) !== null && _c !== void 0 ? _c : [] }));
}
function appendOpsEvent(op, event) {
    var idx = op.events.findIndex(function (item) { return item.id === event.id; });
    if (idx >= 0) {
        op.events[idx] = event;
    }
    else {
    ...
    op.events.sort(function (a, b) { return a.timestamp - b.timestamp; });
...
function attachToolActivityToFeature(op, featureId) {
    var _a;
    var idx = op.features.findIndex(function (item) { return item.id === featureId; });
    if (idx < 0)
        return;
    op.features[idx] = __assign(__assign({}, op.features[idx]), { toolActivity: (_a = featureToolActivity.get(featureId)) !== null && _a !== void 0 ? _a : [] });
}
function findFeatureById(featureId) {
    for (var _i = 0, _a = opsState.values(); _i < _a.length; _i++) {
        var op = _a[_i];
        var feature = op.features.find(function (item) { return item.id === featureId; });
        if (feature) {
...
function resolveFeatureIdForToolEvent(raw, parsedData) {
    var _a;
    var featureId = typeof parsedData.featureId === "string"
    ...
    if (featureId)
        return featureId;
    var runId = typeof parsedData.runId === "string"
        ? parsedData.runId
...
function upsertFeatureToolActivity(featureId, activity) {
    ...
    var index = current.findIndex(function (item) { return item.id === activity.id; });
    var next = __spreadArray([], current, true);
    if (index >= 0) {
        var existing = next[index];
        next[index] = __assign(__assign(__assign({}, existing), activity), { startedAt: existing.startedAt, completedAt: (_b = activity.completedAt) !== null && _b !== void 0 ? _b : existing.completedAt });
    }
    else {
        next.push(activity);
    }
...
function applyOpsEvent(type, opId, payload, streamId, ts) {
    ...
    switch (type) {
        ...
        case "ops.feature.dispatched": {
            ...
            var existing = op.features.find(function (item) { return item.id === featureId_1; });
            ...
            upsertFeature(op, {
                id: featureId_1,
                title: String(payload.title || (existing === null || existing === void 0 ? void 0 : existing.title) || featureId_1),
                status: "in-progress",
                phase: phase,
                assignedModel: String(payload.model || (existing === null || existing === void 0 ? void 0 : existing.assignedModel) || "worker"),
                attempts: Math.max(Number((existing === null || existing === void 0 ? void 0 : existing.attempts) || 0), 1),
                toolActivity: (_b = (_a = featureToolActivity.get(featureId_1)) !== null && _a !== void 0 ? _a : existing === null || existing === void 0 ? void 0 : existing.toolActivity) !== null && _b !== void 0 ? _b : [],
            });
            appendOpsEvent(op, normalizeOpsEvent({
                id: streamId,
                timestamp: ts,
        ...
        case "ops.feature.completed": {
            ...
            var existing = op.features.find(function (item) { return item.id === featureId_2; });
            var status_1 = parseOpsFeatureStatus(payload.status);
            upsertFeature(op, {
                id: featureId_2,
                title: String(payload.title || (existing === null || existing === void 0 ? void 0 : existing.title) || featureId_2),
                status: status_1,
                ...
                toolActivity: (_d = (_c = featureToolActivity.get(featureId_2)) !== null && _c !== void 0 ? _c : existing === null || existing === void 0 ? void 0 : existing.toolActivity) !== null && _d !== void 0 ? _d : [],
            });
            appendOpsEvent(op, normalizeOpsEvent({
                id: streamId,
                timestamp: ts,
    ...
    if (op.features.length > 0 && op.totalPhases === 1) {
        var maxPhase = op.features.reduce(function (max, feature) { return Math.max(max, feature.phase || 1); }, 1);
...
function normalizeOpsSnapshot(snapshot) {
    ...
    var features = Array.isArray(snapshot.features)
        ? snapshot.features.map(function (feature) {
            var _a;
            return ({
                id: String((feature === null || feature === void 0 ? void 0 : feature.id) || ""),
                title: String((feature === null || feature === void 0 ? void 0 : feature.title) || (feature === null || feature === void 0 ? void 0 : feature.id) || "Feature"),
                ...
                assignedModel: String((feature === null || feature === void 0 ? void 0 : feature.assignedModel) || "worker"),
                attempts: Math.max(0, Number(feature === null || feature === void 0 ? void 0 : feature.attempts) || 0),
                toolActivity: (_a = featureToolActivity.get(String((feature === null || feature === void 0 ? void 0 : feature.id) || ""))) !== null && _a !== void 0 ? _a : [],
            });
        })
            .filter(function (feature) { return feature.id; })
        : [];
    return {
...
function syncOpsSnapshots() {
    return __awaiter(this, void 0, void 0, function () {
        var hasActiveOp, activeSnapshot, cursor, keys, _a, nextCursor, batch, values, _i, values_1, rawSnapshot, normalized, existing, merged, shouldBroadcast, err_1;
        var _b;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    if (!streamRedis || shuttingDown)
                        return [2 /*return*/];
                    _c.label = 1;
                case 1:
                    _c.trys.push([1, 8, , 9]);
                    hasActiveOp = Array.from(opsState.values()).some(function (op) {
                ...
                case 2:
                    ...
                    if (!hasActiveOp && !activeSnapshot)
                        return [2 /*return*/];
                    cursor = "0";
                    keys = [];
                    _c.label = 3;
                case 3: return [4 /*yield*/, streamRedis.scan(cursor, "MATCH", "ops:snapshot:*", "COUNT", "100")];
                case 4:
                ...
                case 7:
                    ...
                    for (_i = 0, values_1 = values; _i < values_1.length; _i++) {
                        ...
                        try {
                            ...
                            merged = __assign(__assign(__assign({}, (existing !== null && existing !== void 0 ? existing : normalized)), normalized), { events: (_b = existing === null || existing === void 0 ? void 0 : existing.events) !== null && _b !== void 0 ? _b : [] });
                            merged.status = getOpsStatus(merged);
                            shouldBroadcast = !existing ||
...
function translateEvent(raw, streamId) {
    ...
    var currentProgress = (_a = run === null || run === void 0 ? void 0 : run.progress) !== null && _a !== void 0 ? _a : 0;
    switch (type) {
        case "run.start":
            return __assign(__assign({}, base), { phase: "queued", title: "Starting to work on your request…", icon: "rocket", progress: 5 });
        case "llm.start":
            return __assign(__assign({}, base), { phase: "understanding", title: "Thinking about your request…", icon: "brain", progress: Math.min(currentProgress + 5, 95) });
        case "llm.done":
            return __assign(__assign({}, base), { phase: "working", title: "Figured out the next step…", icon: "lightbulb", isActive: false, progress: Math.min(currentProgress + 5, 95) });
        case "tool.start": {
            ...
            return __assign(__assign({}, base), { title: toolName || friendly.title, icon: friendly.icon, toolName: toolName, toolArgs: typeof toolArgs === "string" ? toolArgs : JSON.stringify(toolArgs), description: friendly.title, progress: Math.min(currentProgress + 3, 95) });
        }
        case "tool.done": {
            var toolName = raw.toolName || raw["data.toolName"] || "";
            var toolArgs = raw.toolArgs || "";
            var friendly = toolFriendlyNames[toolName] || { title: "Completed a step", icon: "check" };
            return __assign(__assign({}, base), { title: toolName || friendly.title.replace("…", " ✓"), icon: "check", toolName: toolName, toolArgs: typeof toolArgs === "string" ? toolArgs : JSON.stringify(toolArgs), description: friendly.title.replace("…", " ✓"), isActive: false, progress: Math.min(currentProgress + 5, 95) });
        }
        case "tool.error":
            return __assign(__assign({}, base), { title: "Hit a small bump — working around it…", icon: "alert-triangle", progress: currentProgress });
        case "subagent.spawn":
            return __assign(__assign({}, base), { title: "A specialist is helping out…", icon: "users", progress: Math.min(currentProgress + 5, 95) });
        case "subagent.done":
            return __assign(__assign({}, base), { title: "Specialist finished their part ✓", icon: "user-check", isActive: false, progress: Math.min(currentProgress + 10, 95) });
        case "run.done": {
            var status_2 = raw.status || raw["data.status"] || "ok";
            if (status_2 === "ok" || status_2 === "done") {
                return __assign(__assign({}, base), { phase: "complete", title: "All done! ✓", icon: "check-circle", isActive: false, progress: 100 });
            }
            return __assign(__assign({}, base), { phase: "error", title: "Something went wrong — we're on it", icon: "alert-triangle", isActive: false, progress: currentProgress });
        }
        default:
            return null;
    }
...
function processEvent(fields, streamId) {
    ...
    var argValue = parsedData.commandPreview ||
        parsedData.command ||
        parsedData.path ||
        parsedData.query ||
        parsedData.pattern ||
    ...
    if (raw.type === "message_sent" && raw.messageType === "TaskRequest") {
        raw.type = "run.start";
        raw.task = ((_a = parsedData === null || parsedData === void 0 ? void 0 : parsedData.payload) === null || _a === void 0 ? void 0 : _a.task) || (parsedData === null || parsedData === void 0 ? void 0 : parsedData.task) || "Working on something…";
    }
    if (raw.type === "message_received" && raw.messageType === "TaskRequest") {
        raw.type = "run.start";
        raw.task = ((_b = parsedData === null || parsedData === void 0 ? void 0 : parsedData.payload) === null || _b === void 0 ? void 0 : _b.task) || (parsedData === null || parsedData === void 0 ? void 0 : parsedData.task) || "New task received…";
    }
    if (raw.type === "message_sent" && raw.messageType === "TaskResult") {
        raw.type = "run.done";
        raw.status = ((_c = parsedData === null || parsedData === void 0 ? void 0 : parsedData.payload) === null || _c === void 0 ? void 0 : _c.status) || "ok";
    }
    var type = raw.type || "";
    var runId = raw.runId || "unknown";
    ...
    if (type === "run.start") {
        var task = raw.task || ((_d = parsedData === null || parsedData === void 0 ? void 0 : parsedData.payload) === null || _d === void 0 ? void 0 : _d.task) || (parsedData === null || parsedData === void 0 ? void 0 : parsedData.task) || raw["data.task"] || "Working on something…";
        var newRun = {
            runId: runId,
            task: task,
            phase: "queued",
    ...
    if (activity.phase === "complete" || activity.phase === "error") {
        setTimeout(function () { return runs.delete(runId); }, 3600000);
    }
...
function startRedisListener() {
    return __awaiter(this, void 0, void 0, function () {
        var STREAM, lastId, recent, sorted, _i, sorted_1, _a, id, fields, err_2, result, _b, result_1, _c, messages, _d, messages_1, _e, id, fields, err_3;
        var _f, _g;
            ...
            switch (_h.label) {
                case 0:
                    streamRedis = new ioredis_1.default(redisUrl, {
                        ...
                        retryStrategy: function (times) { return Math.min(times * 500, 10000); },
                    });
                    streamRedis.on("ready", function () {
                        streamRedisConnected = true;
                        console.log("[redis-stream] connected — listening for telemetry events");
                    });
                    streamRedis.on("connect", function () {
                        streamRedisConnected = true;
                    });
                    streamRedis.on("close", function () {
                        streamRedisConnected = false;
                    });
                    streamRedis.on("end", function () {
                        streamRedisConnected = false;
                    });
                    streamRedis.on("error", function (err) {
                        streamRedisConnected = false;
                        if (!shuttingDown) {
                ...
                case 2:
                    ...
                    if (recent.length > 0) {
                        ...
                        console.log("[redis-stream] caught up on ".concat(recent.length, " recent events (").concat(runs.size, " runs)"));
                    }
                    return [3 /*break*/, 4];
                case 3:
                    err_2 = _h.sent();
                    if (!shuttingDown) {
                        console.log("[redis-stream] catch-up failed, starting from live:", err_2 === null || err_2 === void 0 ? void 0 : err_2.message);
                    }
                    return [3 /*break*/, 4];
                case 4:
                ...
                case 7:
                    ...
                    if (shuttingDown)
                        return [3 /*break*/, 13];
                    if (!(((_f = err_3 === null || err_3 === void 0 ? void 0 : err_3.message) === null || _f === void 0 ? void 0 : _f.includes("ECONNRESET")) || ((_g = err_3 === null || err_3 === void 0 ? void 0 : err_3.message) === null || _g === void 0 ? void 0 : _g.includes("closed")))) return [3 /*break*/, 9];
                ...
                case 8:
                    ...
                    return [3 /*break*/, 11];
                case 9:
                    console.error("[redis-stream] stream read error:", err_3 === null || err_3 === void 0 ? void 0 : err_3.message);
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    shutdownPromise = (function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    _a.sent();
                    void startRedisListener().catch(function (err) {
                        if (!shuttingDown) {
                            console.error("[redis-stream] failed to start listener:", err);
                        }
                    });
                    opsInterval_1 = setInterval(function () {
                        void syncOpsSnapshots().catch(function (err) {
                            if (!shuttingDown) {
                                console.error("[ops-snapshot] periodic sync error:", err === null || err === void 0 ? void 0 : err.message);
                            }
                        });
                    }, 5000);
                    // Initial sync
                    ...
                    process.once("SIGTERM", function () { return clearInterval(opsInterval_1); });
                    process.once("SIGINT", function () { return clearInterval(opsInterval_1); });
                    return [3 /*break*/, 4];
                case 3:
                    err_5 = _a.sent();
