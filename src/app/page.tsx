...
import { Activity, AlertTriangle, CheckCircle2, Coffee, ShieldAlert, Timer, Zap } from "lucide-react";
import { type ReactNode, useMemo } from "react";
import { Shimmer } from "@/components/ai-elements/shimmer";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Separator } from "@/components/ui/separator";
import { useHealth, useRuns, useScores } from "@/hooks/use-api";
...
export default function HomePage() {
	...
	const recentTasks = [...runs].sort((a, b) => (b.lastAt ?? 0) - (a.lastAt ?? 0)).slice(0, 10);
				...
				<div className="rounded-lg border border-red-500/30 bg-red-500/10 p-3 flex items-center gap-2">
					...
					<div>
						...
						<p className="text-xs text-red-200/70">
							{runsQuery?.error ? "Tasks " : ""}
							{scoresQuery?.error ? "Scores " : ""}
							{healthQuery?.error ? "Health " : ""}— showing last available data.
						</p>
					</div>
				</div>
			)}
						...
						{recentActivities.length === 0 ? (
							...
							recentActivities.map((activity) => (
								<div key={activity.id ?? `${activity.runId}-${activity.timestamp}`} className="rounded-md border p-3">
									...
									<p className="mt-1 text-xs text-muted-foreground truncate">{activity.description ?? "In progress"}</p>
								...
								{failedRuns.slice(0, 3).map((run) => (
									<div key={`failed-${run.runId}`} className="rounded-md border border-red-500/30 bg-red-500/10 p-3">
										...
										<p className="text-xs text-red-100/80 truncate">
											{run.taskPreview ?? run.summaryPreview ?? run.runId}
										</p>
									</div>
								))}
