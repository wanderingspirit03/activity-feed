	...
	return (
		...
		>
			<div className="flex items-center gap-2 min-w-0">
				...
				<span className="font-mono text-sm text-neutral-300 shrink-0">{item.toolName || "tool"}</span>
...
export default function LivePage() {
	...
	const activeRuns = runArray.filter((r) => r.status === "working" || r.status === "running");
	const recentRuns = runArray.filter((r) => r.status !== "working" && r.status !== "running");
