...
import { AlertTriangle, Bot, Coffee, Filter, RefreshCcw, Search, Wrench } from "lucide-react";
...
function normalizeStatus(run: RunLike): "running" | "completed" | "failed" | "stuck" {
	...
	if (status.includes("complete") || status.includes("done") || status.includes("ok") || status.includes("success"))
		return "completed";
	if (
...
export default function TasksPage() {
	...
	const filteredAssistants = useMemo(() => {
		...
		return assistants.filter((assistant) =>
			[assistant.address, ...(assistant.capabilities ?? [])].filter(Boolean).join("\n").toLowerCase().includes(q),
		);
	}, [assistants, search]);
											...
											<Card key={run.runId ?? idx} className="border-border/70">
												<CardContent className="pt-5 space-y-3">
													...
													<div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
														...
														<span>
															{durationMs ? `Duration ${formatDuration(durationMs)}` : "Duration not available"}
														</span>
														...
														<ToolHeader
															...
															meta={
																<span className="text-[11px] text-muted-foreground">
																	{run.lastEventType ?? "latest event"}
																</span>
															}
														/>
														<ToolContent>
													...
													<div className="flex items-start justify-between gap-2">
														<div className="min-w-0">
															<p className="font-medium text-red-200">
																{entry.reason ?? "Task failed for an unknown reason"}
															</p>
											...
											<CardContent className="pt-5">
												<div className="flex flex-wrap items-start justify-between gap-2">
													<div className="min-w-0">
														...
														<p className="text-xs text-muted-foreground mt-1">
															{assistant.updatedAt
																? `Last active ${formatRelative(assistant.updatedAt)}`
																: "Last active time not available"}
														</p>
													</div>
