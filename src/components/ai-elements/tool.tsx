...
export const getStatusBadge = (status: ToolState, activePulse?: boolean) => (
	<Badge className={cn("gap-1.5 rounded-full text-xs", activePulse && "animate-pulse")} variant="secondary">
		{statusIcons[status]}
		{statusLabels[status]}
	</Badge>
);

export const ToolHeader = ({ className, title, type, state, toolName, meta, activePulse }: ToolHeaderProps) => {
	const derivedName = toolName || (type ? type.split("-").slice(1).join("-") : "tool");

	return (
		<CollapsibleTrigger className={cn("flex w-full items-center justify-between gap-2 p-3", className)}>
...
export const ToolContent = ({ className, ...props }: ToolContentProps) => (
	<CollapsibleContent className={cn("space-y-4 p-4 text-popover-foreground", className)} {...props} />
);
