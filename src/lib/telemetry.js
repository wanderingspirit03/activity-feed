...
function getApiBaseMaybe() {
	...
	var base =
		(_b =
			(_a = process.env.ACTOR_API_BASE_URL) !== null && _a !== void 0
				? _a
				: process.env.NEXT_PUBLIC_ACTOR_API_BASE_URL) !== null && _b !== void 0
			? _b
			: "";
...
function computePrimaryStatus(run) {
	...
	if (((_a = run.humanInFlight) !== null && _a !== void 0 ? _a : 0) > 0) return "waiting_human";
	if (((_b = run.llmInFlight) !== null && _b !== void 0 ? _b : 0) > 0) return "waiting_llm";
...
function toSummary(run) {
	var _a, _b, _c;
	var summaryPreview =
		(_b = (_a = run.summary) !== null && _a !== void 0 ? _a : run.error) !== null && _b !== void 0 ? _b : undefined;
	...
	return __awaiter(this, void 0, void 0, function () {
		...
		return __generator(this, (_d) => {
			switch (_d.label) {
				...
				case 1:
					ids = _d.sent();
					if (ids.length === 0) return [2 /*return*/, { runs: [], nextCursor: undefined }];
				...
				case 2:
					...
					for (i = 0; i < ids.length; i += 1) {
						runId = ids[i];
						hash =
							(_b = rows === null || rows === void 0 ? void 0 : rows[i]) === null || _b === void 0 ? void 0 : _b[1];
					...
					filtered = runs
						...
						.filter((run) => {
							...
							return (
								((_a = run.primaryStatus) !== null && _a !== void 0 ? _a : computePrimaryStatus(run)) === options.status
							);
						})
						.filter((run) => {
							var _a;
							...
							if (options.namespace === "main") {
								return (
									actor === "/orchestrator" ||
									(!actor.startsWith("/orchestrator/") && !actor.includes("/orchestrator/"))
								);
							}
							return (
								actor.includes("/orchestrator/".concat(options.namespace)) ||
								actor.includes("/".concat(options.namespace, "/")) ||
								actor.includes("/".concat(options.namespace))
							);
						})
						.slice(0, limit);
					nextCursor =
						filtered.length > 0
							? (_c = filtered[filtered.length - 1]) === null || _c === void 0
								? void 0
								: _c.lastAt
							: undefined;
					return [
						2 /*return*/,
						{ runs: filtered, nextCursor: nextCursor !== null && nextCursor !== void 0 ? nextCursor : undefined },
					];
			}
		});
	});
		...
		return __generator(this, (_s) => {
			switch (_s.label) {
				...
				case 3:
					...
					if (!lastId) return [3 /*break*/, 4];
					start = "(".concat(lastId);
					if (chunk.length < 500) return [3 /*break*/, 4];
					return [3 /*break*/, 2];
				case 4:
					events.sort((a, b) => a.ts - b.ts);
					actions = events.filter((event) =>
					...
					for (_c = 0, events_1 = events; _c < events_1.length; _c++) {
						event_1 = events_1[_c];
						messageId =
							typeof ((_j = event_1.data) === null || _j === void 0 ? void 0 : _j.messageId) === "string"
								? event_1.data.messageId
								: event_1.id;
						if (
							event_1.messageType === "TaskRequest" &&
							((_l = (_k = event_1.data) === null || _k === void 0 ? void 0 : _k.payload) === null || _l === void 0
								? void 0
								: _l.task)
						) {
						...
						if (
							event_1.messageType === "TaskResult" &&
							((_o = (_m = event_1.data) === null || _m === void 0 ? void 0 : _m.payload) === null || _o === void 0
								? void 0
								: _o.taskId) === "summary"
						) {
							...
							tasks.push({
								...
								text: String(
									(_r =
										(_q = event_1.data.payload.result) !== null && _q !== void 0 ? _q : event_1.data.payload.error) !==
										null && _r !== void 0
										? _r
										: "",
								),
					...
					actors = Array.from(new Set(events.map((event) => event.actor).filter(Boolean)));
		...
		return __generator(this, (_o) => {
			switch (_o.label) {
				...
				case 6:
					...
					events = ((_d = eventsPayload.events) !== null && _d !== void 0 ? _d : []).map((event, index) => {
						var _a, _b, _c, _d, _e;
						return {
					...
					for (_i = 0, events_2 = events; _i < events_2.length; _i++) {
						event_2 = events_2[_i];
						messageId =
							typeof ((_e = event_2.data) === null || _e === void 0 ? void 0 : _e.messageId) === "string"
								? event_2.data.messageId
								: event_2.id;
						if (
							event_2.messageType === "TaskRequest" &&
							((_g = (_f = event_2.data) === null || _f === void 0 ? void 0 : _f.payload) === null || _g === void 0
								? void 0
								: _g.task)
						) {
							key = "request:".concat(messageId);
						...
						if (
							event_2.messageType === "TaskResult" &&
							((_j = (_h = event_2.data) === null || _h === void 0 ? void 0 : _h.payload) === null || _j === void 0
								? void 0
								: _j.taskId) === "summary"
						) {
							key = "result:".concat(messageId);
							...
							tasks.push({
								...
								text: String(
									(_m =
										(_l = event_2.data.payload.result) !== null && _l !== void 0 ? _l : event_2.data.payload.error) !==
										null && _m !== void 0
										? _m
										: "",
								),
...
function unavailableFullTrace(runId, message, temporarilyUnavailable) {
	if (temporarilyUnavailable === void 0) {
...
function normalizeFullTrace(runId, raw) {
	...
	var sections = sectionsRaw
		.map((entry, index) => {
			...
			var header =
				typeof entry.header === "string" ? scrubDisplayText(entry.header) : "[SECTION_".concat(index + 1, "]");
	...
	return {
		...
		status: raw.status === "ok" || raw.status === "failed" || raw.status === "skipped" ? raw.status : undefined,
		...
		redactionCount:
			typeof raw.redactionCount === "number" && Number.isFinite(raw.redactionCount) ? raw.redactionCount : undefined,
		sections: sections,
		stats: {
			messageCount:
	...
	return __awaiter(this, void 0, void 0, function () {
		...
		return __generator(this, (_j) => {
			switch (_j.label) {
				...
				case 1:
					addresses = _j.sent();
					if (addresses.length === 0) return [2 /*return*/, []];
					pipeline = redis.pipeline();
				...
				case 2:
					...
					for (i = 0; i < results.length; i++) {
						...
						try {
							...
							entries.push({
								...
								name:
									(_f = (_e = parsed.name) !== null && _e !== void 0 ? _e : parsed.address.split("/").pop()) !== null &&
									_f !== void 0
										? _f
										: parsed.address,
					...
					return [2 /*return*/, entries.sort((a, b) => b.updatedAt - a.updatedAt)];
				case 3:
					_a = _j.sent();
					return [2 /*return*/, []];
				case 4:
	...
	return __awaiter(this, arguments, void 0, function (limit) {
		...
		return __generator(this, (_h) => {
			switch (_h.label) {
				...
				case 2:
					chunk = _h.sent();
					if (!chunk || chunk.length === 0) return [3 /*break*/, 3];
					for (_i = 0, _a = chunk; _i < _a.length; _i++) {
						(_b = _a[_i]), (entryId = _b[0]), (pairList = _b[1]);
						fields = pairsToObject(pairList);
						...
						events.push({
							...
							reason:
								(_e =
									(_d = data === null || data === void 0 ? void 0 : data.reason) !== null && _d !== void 0
										? _d
										: data === null || data === void 0
											? void 0
											: data.error) !== null && _e !== void 0
									? _e
									: undefined,
					...
					if (!lastId) return [3 /*break*/, 3];
					start = "(".concat(lastId);
					if (chunk.length < 500) return [3 /*break*/, 3];
					return [3 /*break*/, 1];
				case 3:
