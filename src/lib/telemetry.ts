...
const STUCK_TOOL_MS = Number.isFinite(stuckToolMsRaw) && stuckToolMsRaw > 0 ? Math.max(60_000, stuckToolMsRaw) : 600000;
...
async function getRunsFromRedis(options: {
	...
	const filtered = runs
		...
		.filter((run) => {
			...
			return (
				actor.includes(`/orchestrator/${options.namespace}`) ||
				actor.includes(`/${options.namespace}/`) ||
				actor.includes(`/${options.namespace}`)
			);
		})
		.slice(0, limit);
...
export async function getRuns(options: {
	...
	try {
		...
		const payload = await apiFetch<{ runs: ApiRun[]; nextCursor?: number }>(`/api/runs?${params.toString()}`);
...
function unavailableFullTrace(runId: string, message: string, temporarilyUnavailable = false): FullTraceDetail {
	return {
		available: false,
		source: "none",
